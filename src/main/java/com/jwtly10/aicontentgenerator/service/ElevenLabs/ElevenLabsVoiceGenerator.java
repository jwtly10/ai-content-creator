...
public class ElevenLabsVoiceGenerator implements VoiceGenerator<ElevenLabsVoice> {
    ...
    public ElevenLabsVoiceGenerator(RestTemplate restTemplate) {
     ...
     */
    @Override
    public Optional<String> generateVoice(String text, Gender gender, String outputPath) {
        return switch (gender) {
            case MALE -> generateMp3Voice(createBody(text), "ErXwobaYiN019PkySvjV", outputPath);
            case FEMALE -> generateMp3Voice(createBody(text), "21m00Tcm4TlvDq8ikWAM", outputPath);
        };
    }
     ...
     * @param outputPath Path to output file
     */
    public Optional<String> generateMp3Voice(ElevenLabsBody body, String voiceId, String outputPath) {
        try {
